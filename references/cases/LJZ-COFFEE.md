...
> 2026-06-18 首次深层录入。15 镜头 / 5 场景组 / 9 张精选帧。

### 镜头序列总览

#### 场景1: 开篇 — 咖啡之城登场（00:00 — 00:03.5）

| # | 时间码 | 时长 | 景别 | 构图 | 运镜 | 色调+场景 | 角色 | 音效 | 叙事功能 | 特效 | 分镜图 |
|---|--------|------|------|------|------|----------|------|------|----------|------|--------|
| 1 | 00:00.0-01.5 | 1.5s | 全景 | 引导线 | 平移 | 粉河+咖啡棕 | 小人 | 汽笛+爵士 | 引入主题 | — | [图1] |
| 2 | 01.5-03.5 | 2.0s | 全景 | 对称 | 固定 | 暖橘+樱粉 | 小人 | 鸟鸣+爵士 | 城市氛围 | — | — |

#### 场景2: 咖啡创意 — 器具×建筑融合（00:03.5 — 00:08.5）

| # | 时间码 | 时长 | 景别 | 构图 | 运镜 | 色调+场景 | 角色 | 音效 | 叙事功能 | 特效 | 分镜图 |
| 3 | 03.5-05.5 | 2.0s | 中景 | 中心 | 固定 | 纯白+东方明珠 | 小人 | 液体搅拌声 | 牛奶元素 | — | [图7] |
| 4 | 05.5-06.5 | 1.0s | 中景 | 中心 | 固定 | 银金属+暖 | 小人 | 咖啡滴落声 | 咖啡器具 | — | — |
| 5 | 06.5-08.5 | 2.0s | 全景 | 三分法 | 固定 | 巧克力棕+粉云 | 小人 | 淋酱声 | 甜品元素 | — | — |

#### 场景3: 城市漫游 — 移动中的咖啡城（00:08.5 — 00:16.5）

| # | 时间码 | 时长 | 景别 | 构图 | 运镜 | 色调+场景 | 角色 | 音效 | 叙事功能 | 特效 | 分镜图 |
| 6 | 08.5-11.5 | 3.0s | 中景 | 斜角 | 平移跟随 | 红+咖啡色 | 小人 | 引擎+环境 | 交通工具 | — | [图2] |
| 7 | 11.5-14.5 | 3.0s | 中景 | 中心 | 固定 | 中国红+咖啡棕 | 小人 | 豆散落声 | 文化融合 | — | [图8] |
| 8 | 14.5-16.5 | 2.0s | 中景 | 中心 | 俯拍固定 | 红+咖啡棕 | 小人 | 压粉重音 | 制作过程 | — | — |

#### 场景4: 视觉高潮 — 从喷泉到日落（00:16.5 — 00:27.5）

| # | 时间码 | 时长 | 景别 | 构图 | 运镜 | 色调+场景 | 角色 | 音效 | 叙事功能 | 特效 | 分镜图 |
| 9 | 16.5-18.5 | 2.0s | 全景 | 引导线 | 推镜头 | 梦幻粉+绿植 | 无 | 蒸汽声 | 童话感 | — | [图3] |
| 10 | 18.5-21.5 | 3.0s | 全景 | 中心 | 固定 | 奶白+咖啡棕 | 小人 | 喷泉流水 | 视觉高潮 | 拉花CG | [图4] |
| 11 | 21.5-24.5 | 3.0s | 远景 | 引导线 | 固定 | 暖橘+粉紫霞光 | 无 | 悠扬爵士 | 情感升华 | 逆光 | [图5] |
| 12 | 24.5-27.5 | 3.0s | 全景 | 中心 | 固定 | 奶油白+节日红 | 小人 | 欢呼+乐器 | 十周年庆典 | 烟花CG | [图9] |

#### 场景5: 收尾 — 庆典高潮→品牌（00:27.5 — 00:37.0）

| # | 时间码 | 时长 | 景别 | 构图 | 运镜 | 色调+场景 | 角色 | 音效 | 叙事功能 | 特效 | 分镜图 |
| 13 | 27.5-30.5 | 3.0s | 全景 | 仰拍 | 摇镜头 | 节日缤纷色 | 小人 | 烟花爆炸 | 庆典高潮 | 烟花CG | — |
| 14 | 30.5-33.0 | 2.5s | 特写 | 中心 | 固定 | 黑白线+红点缀 | 插画角色 | 吹蜡烛声 | 品牌联结 | 插画转场 | [图6] |
| 15 | 33.0-37.0 | 4.0s | 特写 | 中心 | 固定 | 白底红字 | 无 | 结尾音效 | 收尾信息 | — | — |

### 叙事节奏（→ Writer）

- 全片 37s / 15 镜头，平均 2.5s/镜头，快节奏
- 节奏曲线：0-8.5s 建立世界（5镜/8.5s, 1.7s均值）→ 8.5-16.5s 中速漫游（3镜/8s, 2.7s）→ 16.5-27.5s 高潮区间（4镜/11s, 2.75s）→ 27.5-37s 收尾（3镜/9.5s, 3.2s缓降）
- 叙事功能分布：引入主题(2)→创意展示(6)→情感升华(1)→庆典高潮(2)→品牌联结(1)→收尾(1)→城市氛围过渡(2)
- 并列式叙事：15 个镜头各自独立呈现咖啡×城市意象，以「十周年庆典蛋糕」汇聚
- 无角色弧线：小人模型是氛围道具，非叙事主体
- 信息密度：画面内置「10.22-26」和「LUJIAZUI COFFEE FESTIVAL」文字贯穿全片

### 镜头语言序列（→ DP）

- 全片 92% 固定机位（14/15），仅 3 处运镜：
  - 镜头1 平移（跟随咖啡豆轮船，建立动线）
  - 镜头6 平移跟随（咖啡杯巴士，唯一移动主体镜头）
  - 镜头9 推镜头（摩卡壶城堡，唯一的纵深运动）
  - 镜头13 摇镜头（飞艇烟花，仰拍旋转）
- 景别分布：全景 8 镜(53%) / 中景 5 镜(33%) / 特写 2 镜(14%) / 远景 1 镜 — 全景为主，符合微缩模型「看全貌」的特性
- 构图偏好：中心构图 8 镜(53%) / 引导线 3 镜(20%) / 对称 1 镜 / 三分法 1 镜 / 斜角 1 镜 / 仰拍 1 镜 — 中心构图占绝对主导，因为每个镜头的主视觉是「巨型咖啡道具」
- 景深：全片浅景深模拟移轴/微缩效果（Tilt-shift），中远景虚化严重，焦点集中在道具主体
- 视角：大部分平视/微俯拍，镜头8 为俯拍（压粉器），镜头13 为仰拍（飞艇）

### 色调+场景搭建序列（→ Art Director）

- **核心四色体系**：咖啡棕 / 牛奶白 / 樱花粉 / 中国红 — 全片统一
- 调色参数（模拟胶片风格）：暖调基底、低对比、高明度（粉色调提升甜美感）
- 前段（00:00-08.5s）：粉河+樱粉+暖橘 — 童话感引入
- 中段（08.5-16.5s）：红+咖啡色 — 城市活力
- 后段（16.5-27.5s）：暖橘逆光→奶油白节日 — 情感爬升
- 收尾（27.5-37.0s）：黑白线+红点缀→白底红字 — 品牌归位
- 光源策略：白天场景多为均匀自然光/漫反射，仅镜头11 使用逆光（日落），镜头8 使用顶光（俯拍）
- **场景搭建核心逻辑**：咖啡器具放大为建筑尺度融合城市地标——牛奶搅拌站×东方明珠、摩卡壶=凉亭、咖啡杯=巴士、咖啡豆斗拱=中国馆、压粉器=穹顶、拉花缸=喷泉。每个场景的空间布局遵循「巨型道具居中+小人围绕+陆家嘴建筑背景」三要素
- 视觉重心：始终在巨型咖啡道具上，小人模型提供尺度参照

### 角色设计序列（→ Writer + Art Director + DP + Sound）

- 本片无核心角色。小人模型作为尺度锚点和氛围道具出现（12/15 镜头），无个体身份
- 镜头14 插画角色（黑白双人）作为品牌符号出现，仅此一次
- Writer 注意事项：如需为类似项目设计角色，角色应为「咖啡师」「游客」「居民」等城市角色，保持无对白、群像化

### 音效序列（→ Sound）

- 配乐：全片统一轻快爵士（BPM ~120-130），无变奏，从开篇到收尾连续覆盖
- 音效策略：Foley 拟音高度精细化——每个咖啡动作都有对应音效
  - 镜头1：汽笛声（咖啡豆轮船）
  - 镜头3：液体搅拌/倾倒声
  - 镜头6：引擎声（咖啡巴士）
  - 镜头7：咖啡豆散落声
  - 镜头8：压粉重音（打击乐感，唯一节奏断点）
  - 镜头10：喷泉流水声
  - 镜头12-13：欢呼声+烟花爆炸声（庆典密度最高区间）
  - 镜头14：吹灭蜡烛声（转场信号）
- 无声段落：无。全片音乐+音效连续覆盖
- 节奏断点：镜头8 的压粉声是唯一打破匀质节奏的打击音

### 特效序列（→ VFX）

- 核心特效技法：**微缩模型+移轴摄影**（浅景深模拟）— 全片基底
- CG 合成点（5 处）：
  - 镜头1：粉色咖啡河（流体CG） + 飞艇（CG）
  - 镜头10：拉花形成动画（流体CG，喷泉→拉花图案渐变）
  - 镜头12-13：烟花粒子（粒子CG）
  - 镜头14：插画风格转场（2D 动画→品牌信息）
- 转场方式：14/15 镜头使用硬切（快速剪辑节奏），仅镜头14→15 使用插画图形转场
- 材质：微缩模型以塑料/树脂/木材为主（实体手作质感），CG 元素追求与实体模型的视觉一致性
