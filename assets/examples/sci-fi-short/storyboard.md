...
|------|------|
| **景别** | extreme-wide |
| **画面描述** | 建立镜头：尘橙天空下的巨型粗野主义建筑群。灰烬粒子漂浮。飞行车在建筑间穿行（占画面<3%）。残破的全息广告牌在建筑侧面闪烁。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 24mm广角，极缓dolly推进+微俯摇（12秒），单一尘橙太阳光源 |
| **特效备注** | 全息广告牌不完美系统+灰烬粒子+volumetric fog |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | medium |
| **画面描述** | 记忆贩的巢穴——逼仄的混凝土空间。他（背影）坐在工作台前，冷蓝色设备光是唯一光源。工作台上散落着带手写标签的记忆晶片。 |

#### AI 生成提示词
```
medium shot of a small concrete apartment interior, a man in silhouette sitting at a workbench facing away from camera, single cold blue light source from a neural interface device, scattered memory chips with handwritten faded labels on the workbench, cracked concrete walls, narrow claustrophobic space, Blade Runner 2049 K's apartment aesthetic, cinematic, moody, 2.39:1
```

| 项目 | 内容 |
|------|------|
| **镜头备注** | 35mm→85mm（从空间建立到工作台推进），dolly极缓推进，单一冷蓝设备光源 |
| **特效备注** | 设备光数据波纹+晶片插入的蓝白火花 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | close-up |
| **画面描述** | 记忆回放中的异常画面——一棵真实的树在绿色叶子在阳光下摇曳，背景是蓝色的天空。画质有旧胶片式的色彩偏移和边缘VHS噪点。这是全片最饱和的画面。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 28mm（主观POV）+ 50mm（树），微晃手持感，真实阳光 |
| **特效备注** | 记忆视觉滤镜——色彩偏移+闪烁+VHS噪点+突然高饱和度 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | extreme-wide |
| **画面描述** | 窗外视角——巨型建筑在黄昏中。一个巨大的机械臂以极缓速度在建筑表面移动。建筑占画面70%。尘橙暮光从窗外射入。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 24mm广角，dolly推向窗口→向上摇至巨物，尘橙暮光为主光+冷蓝室内光为辅 |
| **特效备注** | 巨物缓动（10秒+）+黄昏→夜晚过渡+建筑维护红灯闪烁 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | extreme-close-up |
| **画面描述** | 记忆贩的第一次全脸特写——双眼布满血丝，极浅景深。眼神中映出绿色记忆的微弱反光。粗糙的混凝土墙在背景虚化。这是角色情感的最高点。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 85mm，静态，f/1.4极浅景深，冷蓝设备光为主（来自下方），眼中有绿色记忆反光 |
| **特效备注** | 眼中绿色反光（后期合成）+bloom光晕 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | extreme-close-up |
| **画面描述** | 从混凝土裂缝中取出的锈蚀全息投影器。他的手握着投影器，表面有锈迹和划痕。投影器投射出破碎的信息——坐标、日期、'绿色存在'四字。淡绿色投影光照在他手上。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 100mm微距，dolly推进至手部大特写，淡绿全息光为新主光源 |
| **特效备注** | 全息不完美系统+淡绿光面部映射+扫描线 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | wide |
| **画面描述** | 窗外远景——城市边缘的巨型垃圾处理厂燃烧着。暖橙色的火光辉光照亮天空1/3，烟柱升腾。远处的建筑表面被火光映成暖橙色。冷蓝设备光在室内前景。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 24mm广角，从室内工作台快速拉远至窗外远景，冷蓝+暖橙双光交汇 |
| **特效备注** | 火光辉光+烟柱粒子+建筑表面火光映照 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
...
|------|------|
| **景别** | medium |
| **画面描述** | 结尾画面——门框构图。记忆贩的背影在尘橙雾霾中远去。空荡荡的公寓内，工作台上遗留着一枚裂了一角的晶片，标签上写着'2089.4.15 绿色'。冷蓝色设备光仍在闪烁。 |
...
| 项目 | 内容 |
|------|------|
| **镜头备注** | 50mm（门框构图）→100mm微距（最后晶片），静态收束，冷蓝+尘橙双光 |
| **特效备注** | volumetric fog（他消失在雾霾中）+晶片灰尘粒子+裂痕 |
| **关联角色** | — |
| **参考图** | — |
| **状态** | ✅ 已审核 |
